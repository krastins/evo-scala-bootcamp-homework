...
object DataStructures {
  ...
  def naiveSortConsideringEqualValues[T](map: Map[T, Int]): List[(Set[T], Int)] = {
    map.toList
      .sortBy({ case (_, value) => value })
      .foldLeft(List[(Set[T], Int)]()) { (acc, tuple) =>
        tuple match {
          case (key, value) =>
            if (acc.size != 0) {
              val (lastKeys, lastVal) = acc.last
              if (lastVal == value) {
                acc.updated(acc.size - 1, acc.last.copy(_1 = lastKeys + key))
              } else {
                acc.appended((Set(key), value))
              }
            } else {
              acc.appended((Set(key), value))
            }
  ...
  // after I learned groupBy exists
