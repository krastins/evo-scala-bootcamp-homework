...
object EffectsHomework1 {
  ...
  final class IO[A] private(private val run: () => A) {
    ...
    def handleErrorWith[AA >: A](f: Throwable => IO[AA]): IO[AA] = IO(Try(run()).fold(f, IO[AA](_)).run())

    def redeem[B](recover: Throwable => B, map: A => B): IO[B] = IO(Try(run()).fold(recover, map))

    def redeemWith[B](recover: Throwable => IO[B], bind: A => IO[B]): IO[B] = IO(Try(run()).fold(recover, bind).run())
